...
import { searchSemantic as runSemanticSearch, searchText as runTextSearch } from "./search";
...
function fail(error: unknown): ToolResult {
  if (error instanceof FilesystemAccessError) return { ok: false, error: error.message };
...
function countLanguages(entries: readonly { readonly relativePath: string; readonly type: string }[]) {
...
function modeFor(query: string, mode: FindFilesArgs["mode"]): Exclude<FindFilesArgs["mode"], undefined | "auto"> {
  if (mode !== undefined && mode !== "auto") return mode;
  if (/[\\/[*?\]]/.test(query)) return query.includes("*") || query.includes("?") ? "glob" : "exact";
  if (query.startsWith(".") || /^[A-Za-z0-9]+$/.test(query)) return "fuzzy";
  return "fuzzy";
}

function resultEntry(root: string, path: string, type: string, size: number, score?: number) {
  return {
...
function collapseLines(
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (options.collapseImports && /^\s*import\b/.test(text)) {
      const start = i;
      while (i + 1 < lines.length && /^\s*import\b/.test(lines[i + 1] ?? "")) i += 1;
...
async function syntaxCheck(path: string, content: string): Promise<{ ok: boolean; error?: string }> {
...
async function readWholeFile(path: string, maxBytes: number): Promise<{ text: string; version: string }> {
...
function checkExpectedVersion(current: string, expected: string | undefined): ToolResult | null {
  ...
  const repoMap = defineTool<RepoMapArgs>({
    ...
    async execute(args) {
      try {
        ...
        const maxEntries = bounded(args.max_entries, policy.maxEntries, 1, 10_000);
        ...
        const symbols = args.include_symbols === true
          ? (await symbolsForPath(policy, root.path, { recursive: true, maxResults: 100 })).results
          : undefined;
        return {
          ok: true,
          content: {
    ...
    async execute(args) {
      try {
        ...
        if (mode === "extension") {
          ...
          matches = matches.filter((entry) => entry.type === "file" && extname(entry.relativePath) === ext);
        } else if (mode === "regex") {
          ...
          return {
            ...
            content: {
              root: root.path,
              mode,
              results: ranked.map((item) => resultEntry(
                root.path,
                item.item.path,
                item.item.type,
                item.item.size,
    ...
    async execute(args) {
      try {
        ...
        const result = await runTextSearch(root.path, {
          ...
          maxPreviewChars: bounded(args.max_preview_chars, 12_000, 100, 100_000),
        });
    ...
    async execute(args) {
      try {
        ...
        if (args.symbol !== undefined) {
          const found = (await symbolsForFile(resolved.path, resolved.root, maxBytes))
            .find((symbol) => symbol.name === args.symbol);
          if (found === undefined) return { ok: false, error: `symbol ${JSON.stringify(args.symbol)} not found` };
    ...
    async execute(args) {
      try {
        ...
        if (typeof args.anchor === "number") {
          ...
          const symbol = (await symbolsForFile(resolved.path, resolved.root, policy.maxReadBytes))
            .find((item) => item.name === args.anchor);
          anchorLine = symbol?.startLine ?? Math.max(1, lines.findIndex((line) => line.includes(args.anchor as string)) + 1);
        }

        const direction = args.direction ?? "center";
        const start = direction === "next"
          ? anchorLine + 1
          : direction === "prev"
    ...
    async execute(args, ctx) {
      try {
        ...
        for (let i = 0; i < occurrence; i += 1) {
          ...
          if (index === -1) return { ok: false, error: `old_text occurrence ${occurrence} not found` };
        ...
        const validation = await runValidation(config, args.validate, [{ path: resolved.path, content: next }], dirname(resolved.path), ctx);
        return {
          ok: true,
          content: {
            path: displayPath(resolved.root, resolved.path),
  ...
  const editRange = defineTool<EditRangeArgs>({
    ...
    async execute(args, ctx) {
      try {
        ...
        if (args.end_line < args.start_line) return { ok: false, error: "end_line must be >= start_line" };
        if (args.start_line > lines.length) return { ok: false, error: "start_line is beyond end of file" };
  ...
  const applyPatch = defineTool<ApplyPatchArgs>({
    ...
    async execute(args, ctx) {
      try {
        ...
        if (parsed.length === 0) return { ok: false, error: "patch did not contain any file changes" };
        ...
        for (const patch of parsed) {
          ...
          if (relPath === null) return { ok: false, error: "patch file is missing a path" };
          const resolved = await resolvePath(policy, relPath, { base: root.path, forCreate: true });
          const current = existsSync(resolved.path) ? await readWholeFile(resolved.path, policy.maxWriteBytes) : { text: "", version: fileVersion("") };
          ...
          if (next === false) return { ok: false, error: `patch failed to apply to ${relPath}` };
          ...
          if (!deleting) changedFiles.push({ path: resolved.path, content: next });
          files.push({
            path: relPath,
            changed: true,
            deleted: deleting,
        ...
        const validation = dryRun
          ? undefined
          : await runValidation(config, args.validate, changedFiles, root.path, ctx);
  ...
  const writeFileTool = defineTool<WriteFileArgs>({
    ...
    async execute(args) {
      try {
        ...
        if (mode === "create_only" && exists) return { ok: false, error: "file already exists" };
        if ((args.create_dirs ?? true) === false && !existsSync(dirname(resolved.path))) {
        ...
        if ((args.create_dirs ?? true) === true) await mkdir(dirname(resolved.path), { recursive: true });
        const nextVersion = await atomicWrite(resolved.path, next);
        return {
          ok: true,
          content: {
