...
function regexSymbols(
  ...
  const patterns: Array<{
    ...
    {
      kind: "import",
      regex: /^\s*import\s+(?:type\s+)?(?:.+?\s+from\s+)?["']([^"']+)["']/gm,
      nameIndex: 1,
    },
    {
      kind: "export",
      regex:
        /^\s*export\s+(?:default\s+)?(?:\{[^}]+\}|(?:class|function|interface|type|const|let|var)\s+([A-Za-z_$][\w$]*)?)/gm,
      nameIndex: 1,
    },
    {
      kind: "class",
      regex: /^\s*(?:export\s+)?class\s+([A-Za-z_$][\w$]*)/gm,
      nameIndex: 1,
    },
    {
      kind: "function",
      regex: /^\s*(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)/gm,
      nameIndex: 1,
    },
    {
      kind: "interface",
    ...
    {
      kind: "type",
      regex: /^\s*(?:export\s+)?type\s+([A-Za-z_$][\w$]*)/gm,
      nameIndex: 1,
    },
    {
      kind: "variable",
      regex: /^\s*(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)/gm,
      nameIndex: 1,
    },
  ];
...
async function typescriptSymbols(
  text: string,
  path: string,
  kinds?: readonly SymbolKind[],
): Promise<SymbolInfo[]> {
  ...
  try {
    ...
    const scriptKind = extname(path).toLowerCase().includes("x")
      ? ts.ScriptKind.TSX
      : ts.ScriptKind.TS;
...
export async function symbolsForPath(
  ...
  if (info.isDirectory()) {
    ...
    return {
      ...
      truncated: listed.truncated || out.length > maxResults,
      root: root.path,
    };
  }

  const results = await symbolsForFile(
    resolved.path,
    resolved.root,
    policy.maxReadBytes,
    options.kinds,
