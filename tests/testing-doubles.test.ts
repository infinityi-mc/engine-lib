...
describe("textResult / toolCallResult builders", () => {
  it("builds a buffered text result with optional usage", () => {
    const r = textResult("hello", { inputTokens: 1, outputTokens: 2, totalTokens: 3 });
  ...
  it("builds a tool-call result mirrored as a message part", () => {
    const r = toolCallResult([{ id: "c1", name: "search", arguments: { q: "x" } }]);
    expect(r.finishReason).toBe("tool_calls");
    expect(r.toolCalls).toHaveLength(1);
    expect(r.message.content).toEqual([{ type: "tool_call", id: "c1", name: "search", arguments: { q: "x" } }]);
  });
});

describe("scriptedProvider", () => {
  it("returns each result in turn, then repeats the last", async () => {
    const provider = scriptedProvider([textResult("first"), textResult("second")]);
    const out = async () => (await provider.complete({ messages: [user("go")] })).message.content;
  ...
  it("forwards mockProvider options (e.g. name)", () => {
    expect(scriptedProvider([textResult("x")], { name: "scripted" }).name).toBe("scripted");
  });
});
