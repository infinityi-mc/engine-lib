...
import { buildChatBody, parseChatResponse } from "../../src/providers/openai-compatible/map";
...
describe("buildChatBody", () => {
  it("maps roles to chat messages", () => {
    const toolCall: ToolCallPart = { type: "tool_call", id: "c1", name: "f", arguments: { a: 1 } };
    const req: CompletionRequest = {
      messages: [system("sys"), user("hi"), assistant([toolCall]), toolResult("c1", "ok")],
    };
    ...
    const events = await collect(
      translateChatStream(
        sseMessages(
          ...
          { data: JSON.stringify({ choices: [{ delta: { tool_calls: [{ index: 0, id: "c1", function: { name: "f", arguments: '{"a"' } }] } }] }) },
