...
import { buildAnthropicBody, parseAnthropicResponse } from "../../src/providers/anthropic/map";
...
describe("buildAnthropicBody", () => {
  it("hoists system, defaults max_tokens, and maps a user turn", () => {
    ...
    const body = buildAnthropicBody(req, "claude", false) as Record<string, any>;
  ...
  it("folds a tool result into a user turn with tool_result block", () => {
    const toolCall: ToolCallPart = { type: "tool_call", id: "tu_1", name: "f", arguments: { a: 1 } };
...
describe("parseAnthropicResponse", () => {
  it("parses text + tool_use + usage and maps stop_reason", () => {
    ...
    expect(result.usage).toEqual({ inputTokens: 3, outputTokens: 4, totalTokens: 7, cachedInputTokens: 2 });
  });
});
    ...
    const events = await collect(
      translateAnthropicStream(
        sseMessages(
          ...
          { event: "content_block_start", data: JSON.stringify({ type: "content_block_start", index: 0, content_block: { type: "tool_use", id: "tu_1", name: "f" } }) },
          ...
          { event: "message_delta", data: JSON.stringify({ type: "message_delta", delta: { stop_reason: "tool_use" }, usage: { output_tokens: 7 } }) },
